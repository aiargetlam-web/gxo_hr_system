    ...
    <div className="card">
      <div className="flex-wrap-mobile">
        ...
        {(user?.role === 'hr' || user?.role === 'admin') && (
          <button onClick={() => window.open(`${import.meta.env.VITE_API_URL}/export/communications`)} className="btn btn-outline" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ⬇️ Esporta CSV
          </button>
        )}
      </div>
          ...
          <tbody>
            {comms.map(c => (
              <tr key={c.id}>
                ...
                <td style={{ padding: '0.75rem' }}>{c.user?.email || c.user_id}</td>
            ...
            {comms.length === 0 && <tr><td colSpan={5} style={{ padding: '1rem', textAlign: 'center' }}>Nessuna comunicazione trovata.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
